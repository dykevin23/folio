...
```
regions          ← 행정구역 (self-referencing, 시/구/동 depth 불규칙 대응)
  └── zones      ← 생활권 (사용자 임의 정의)
       └── complexes          ← 아파트 단지
...
시/구/동의 depth가 불규칙하므로 **self-referencing 단일 테이블**로 처리한다.

```sql
create table regions (
  id         uuid primary key default gen_random_uuid(),
  parent_id  uuid references regions(id),   -- 최상위는 null
  name       text not null,
  type       text not null,                 -- 특별시 | 광역시 | 도 | 시 | 군 | 구 | 동
  depth      integer not null               -- 0 = 최상위 (시/도)
);
```
...
> 시/군/구/동 공공 데이터는 외부 소스에서 import하여 초기 세팅한다.
